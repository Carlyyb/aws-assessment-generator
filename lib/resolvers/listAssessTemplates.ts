...
export function request(ctx) {
  ...
  return {
    ...
    limit: ctx.args && ctx.args.limit ? ctx.args.limit : 50,
    nextToken: ctx.args && ctx.args.nextToken ? ctx.args.nextToken : null
  };
}
...
export const response = (ctx) => {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    if (!isInvalid) {
      ...
      if (item.status && validStatuses.indexOf(item.status) === -1) {
